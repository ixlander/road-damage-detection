...
def threshold_sweep(default_score: float) -> list[dict[str, float]]:
    ...
    for t in (0.1, 0.25, 0.5, 0.75):
        rows.append(
            {
                "threshold": t,
                "precision": max(0.0, default_score - (t * 0.05)),
                "recall": max(0.0, default_score - (t * 0.03)),
            }
        )
    return rows
...
def run_evaluate(config_path: Path, out_dir: Path, smoke: bool = False) -> int:
    ...
    summary = EvalSummary(
        map50=baseline, precision=baseline - 0.05, recall=baseline - 0.04, per_class_ap=per_class
    )
